...
namespace BaroqueUI
{
    abstract class BaroqueVRManager
    {
        ...
        internal abstract Controller.Model GetControllerModel(Controller ctrl);
        ...
        internal abstract bool NeedsReloading();
        ...
        protected GameObject LoadCameraRig(string name)
        {
            ...
            var builder = rig.GetComponent<OVRCameraRigBuilder>();
            if (builder != null)
                rig = builder.Build();
            else
    ...
    class Baroque_SteamVRManager : BaroqueVRManager
    {
        ...
        internal override bool NeedsReloading()
        {
            return !steamvr_manager;
        }

        internal override Controller.Model GetControllerModel(Controller ctrl)
        {
            var system = Valve.VR.OpenVR.System;
            if (system == null)
                return Controller.Model.NotReady;

            var index = TrackedObjectIndex(ctrl);
            if (index == unchecked((uint)-1))
                return Controller.Model.NotReady;

            var error = Valve.VR.ETrackedPropertyError.TrackedProp_Success;
            const int capacity = 64;
            var buffer = new System.Text.StringBuilder(capacity);
            var result = system.GetStringTrackedDeviceProperty(index, Valve.VR.ETrackedDeviceProperty.Prop_RenderModelName_String, buffer, capacity, ref error);
            if (result <= 0)
                return Controller.Model.NotReady;

            var s = buffer.ToString();
            if (s == "vr_controller_vive_1_5")
                return Controller.Model.HTCVive;
            else if (s.StartsWith("{indexcontroller}"))
                return Controller.Model.ValveIndex;
            else
                return Controller.Model.Other;
        }
        ...
        internal override bool UpdateControllerState(Controller ctrl, ControllerState ctrl_state)
        {
            ...
            ctrl_state.touchpad_pressed = (controllerState.ulButtonPressed &
                ((1UL << ((int)Valve.VR.EVRButtonId.k_EButton_SteamVR_Touchpad)) |
                 (1UL << ((int)Valve.VR.EVRButtonId.k_EButton_A)))) != 0;
            ...
            if (ctrl.controllerModel == Controller.Model.ValveIndex)
            {
                /* uh, the default mapping of buttons is SERIOUSLY OBSCURE and it doesn't
                 * distinguish, for NO REASON AT ALL, between gripping the handle strongly
                 * and pressing the A button.  IT DOES NOT MAKE SENSE.  I can't find a
                 * workaround at all, and I don't feel like upgrading the whole thing to
                 * SteamVR 2.0.  So we can't use the A button, and we use the B button
                 * instead.  MEH.
                 *
                 * Note that the B button is not mapped to k_EButton_B either, any more
                 * than the A button is not mapped to k_EButton_A.  It's mapped to
                 * k_EButton_ApplicationMenu.  Of course.
                 */
                ctrl_state.touchpad_pressed |= ctrl_state.menu_pressed;
                ctrl_state.menu_pressed = false;
            }
    ...
    class Baroque_OculusVRManager : BaroqueVRManager
    {
        ...
        internal override Controller.Model GetControllerModel(Controller ctrl)
        {
            return Controller.Model.OculusTouch;
        }
        ...
        ComponentColors GetComponentColors(Controller ctrl)
        {
            ...
            if (cc.tex == null)
            {
                cc.tex = new Texture2D(4, 4, TextureFormat.RGB24, false, linear: false);
        ...
        private IntPtr m_nativeBuffer = IntPtr.Zero;

        internal override void HapticPulse(Controller ctrl, int durationMicroSec)
        {
            /* mapping from durationMicroSec, which is a value up to ~500, down to a number of
             * samples */
            int value = 1 + durationMicroSec / 80;
            if (value > 15)
                value = 15;

            if (OVRHaptics.Config.SampleSizeInBytes != 1)
            {
                /* see OVRHapticsClip.cs, the line that says "TODO support multi-byte samples".
                 * This logic here is just to check that it's still the case---and also to
                 * avoid sending haptic pulses if Load() says SampleSizeInBytes == 0, i.e.
                 * not initialized yet (I think because we have only seen hands, not
                 * controllers) */
                OVRHaptics.Config.Load();
                if (OVRHaptics.Config.SampleSizeInBytes != 1)
                {
                    if (OVRHaptics.Config.SampleSizeInBytes > 1)
                        Debug.LogError("OVRHaptics.Config.SampleSizeInBytes > 1");
                    return;
                }
            }
            if (m_nativeBuffer == IntPtr.Zero)
            {
                /* allocate more than 1 byte, just in case the lower-level functions are buggy too */
                const int SIZE = 64;
                m_nativeBuffer = System.Runtime.InteropServices.Marshal.AllocHGlobal(SIZE);
                if (m_nativeBuffer == IntPtr.Zero)
                {
                    Debug.LogError("HapticPulse: AllocHGlobal() == 0");
                    return;
                }
                for (int i = 0; i < SIZE; i++)
                    System.Runtime.InteropServices.Marshal.WriteByte(m_nativeBuffer, i,
                        (byte)((i & 1) == 0 ? 127 : -128));
            }

            OVRPlugin.HapticsBuffer hapticsBuffer;
            hapticsBuffer.Samples = m_nativeBuffer;
            hapticsBuffer.SamplesCount = value;

            uint m_controller;
            switch (ctrl.index)
            {
                case 0: m_controller = (uint)OVRPlugin.Controller.LTouch; break;
                default: m_controller = (uint)OVRPlugin.Controller.RTouch; break;
            }
    ...
    class Baroque_MissingManager : BaroqueVRManager
    {
        ...
        internal override void HapticPulse(Controller ctrl, int durationMicroSec) { }
        internal override void InitializeControllers() { }
        internal override bool NeedsReloading() { return false; }
        ...
        internal override Controller.Model GetControllerModel(Controller ctrl)
        {
            return Controller.Model.NoVRFound;
        }
    }
    ...
    public static class Baroque
    {
        ...
        static internal void _TakeScreenshot(string filename = null)
        {
            Texture2D CopyTexture(RenderTexture source)
            {
            ...
            var head_transform = GetHeadTransform();
            var go = new GameObject();
            go.transform.SetPositionAndRotation(head_transform.position, head_transform.rotation);

            var camera = go.AddComponent<Camera>();
            var render_tex = new RenderTexture(1920, 1080, 0, RenderTextureFormat.ARGB32);
            camera.targetTexture = render_tex;

#if !UNITY_ANDROID
            var pp = head_transform.GetComponent<UnityEngine.PostProcessing.PostProcessingBehaviour>();
            if (pp != null)
            {
                var pp1 = go.AddComponent<UnityEngine.PostProcessing.PostProcessingBehaviour>();
                pp1.profile = pp.profile;
            }
#endif

            camera.Render();

            Texture2D tex1 = CopyTexture(render_tex);
            byte[] rawdata = ImageConversion.EncodeToPNG(tex1);
            ...
            if (filename == null)
            {
                ...
                while (System.IO.File.Exists(
                        filename = Application.persistentDataPath + "/ScreenShot" + i + ".png"))
                    i += 1;
            }
            try
            {
                System.IO.File.WriteAllBytes(filename, rawdata);
                Debug.LogWarning("Wrote " + filename);
            }
            catch (Exception e)
            {
