...
namespace BaroqueUI
{
    ...
    {
        Trigger, Touchpad,   /* Touchpad is mapped to the A or X button on Oculus */
        Grip, Menu,          /* Menu is mapped to the B or Y button on Oculus */
        TouchpadTouched,     /* only on Vive */
    }
    ...
    public class Controller : MonoBehaviour
    {
        ...
        public bool haveJoystick { get { return controllerModel == Model.OculusTouch; } }
        ...
        public float triggerVariablePressure { get { return controllerState.trigger_variable_pressure; } }
        ...
        public Vector2 touchpadPosition { get { return controllerState.touchpad_position; } }

#if UNITY_EDITOR
        public static bool debugPressed { get { return debug_pressed_time != -1f; } }
#else
        public const bool debugPressed = false;
#endif
        ...
        public void HapticPulse(int durationMicroSec = 500)
        {
            if (Application.isPlaying)
                baroque_vr_manager.HapticPulse(this, durationMicroSec);
        }
        ...
        public T GetAdditionalData<T>(ref T[] locals) where T: class, new()
        {
        ...
        Vector3 pos_to_cursor;
        BaroqueVRManager.ControllerState controllerState;
        ...
        public Transform hotspot { get; private set; }
        Dictionary<ControllerTracker, float> overlapping_trackers;
        static List<ControllerTracker> called_controllers_update;

        static internal bool menu_button_for_screenshot;

        internal static void _InitControllers(BaroqueVRManager mgr)
        {
            baroque_vr_manager = mgr;
        ...
        void UpdateControllerModel()
        {
            if (!isActiveAndEnabled)
                return;

            var new_model = baroque_vr_manager.GetControllerModel(this);
            if (new_model == controller_model)
                return;

            Debug.Log("using controller model " + new_model);
            controller_model = new_model;
            ...
            {
                case Model.HTCVive:
                    hotspot.localPosition = new Vector3(0, -0.0358f, 0.011f);
                    hotspot.localRotation = Quaternion.Euler(58.5f, 0, 0);
                    break;
                ...
                case Model.ValveIndex:
                    /* when viewing the controllers: */
                    if (index == 0)
                    {
                        hotspot.localPosition = new Vector3(0.0116f, -0.0138f, -0.0214f);
                        hotspot.localRotation = Quaternion.Euler(22f, 13f, 7f);
                    }
                    else
                    {
                        hotspot.localPosition = new Vector3(-0.0116f, -0.0138f, -0.0214f);
                ...
                default:
                    hotspot.localPosition = new Vector3(0, -0.006f, 0.036f);   /* default */
                    break;
            }
        }
        ...
        internal void _Initialize(int index)
        {
            ...
            controllerState = baroque_vr_manager.NewControllerState(this);

            hotspot = new GameObject("HotSpot").transform;
            hotspot.SetParent(transform);
            hotspot.localScale = Vector3.one;
            hotspot.localRotation = Quaternion.identity;
        ...
        {
            return hotspot.rotation;
        }
        protected virtual bool UpdateControllerState()
        {
            return baroque_vr_manager.UpdateControllerState(this, controllerState);
        }
...
#if UNITY_EDITOR
        static float debug_pressed_time = -1f;
        static float debug_pressed_last;
#endif

        static void PrepareDebugState()
        {
#if UNITY_EDITOR
            debug_pressed_last = debug_pressed_time;
#endif
        }
        ...
        {
#if UNITY_EDITOR
            if (debug_pressed_time == Time.time)
            {
                if (debug_pressed_last != -1f)
                    Debug.Break();
            }
            else
                debug_pressed_time = -1f;
#endif
        }
        ...
        void ReadControllerState()
        {
            ...
            else
            {
                ...
                bool trigclick = (controllerState.trigger_variable_pressure > 0.999f);   /* == 1.0: the controller fully 'clicked' */
                bool trigger = controllerState.trigger_pressed;
                bool pad = controllerState.touchpad_pressed;
                bool padtouch = controllerState.touchpad_touched || pad;
                bool grip = controllerState.grip_pressed;
                bool menu = controllerState.menu_pressed;
#if UNITY_EDITOR
                if (menu && grip)
                {
                    debug_pressed_time = Time.time;
                    menu = false;
                    grip = false;
                }
#endif
                 ...
                 */
                if ((bitmask_buttons & 0x400000U) != 0 && !trigclick && trigger)
                {
                ...
                uint b = 0;
                if (menu) b |= (1U << (int)EControllerButton.Menu);
                if (grip) b |= (1U << (int)EControllerButton.Grip);
                if (pad) b |= (1U << (int)EControllerButton.Touchpad);
                if (trigger) b |= (1U << (int)EControllerButton.Trigger);
                if (padtouch) b |= (1U << (int)EControllerButton.TouchpadTouched);
            ...
            if (is_tracking_active)
            {
                ...
                current_rotation = ComputeRotation();
...
#if UNITY_EDITOR
            if (menu_button_for_screenshot && btn == EControllerButton.Menu)
                Baroque._TakeScreenshot();
#endif
            return tracker;
        }
        ...
        void UpdateScrollWheel()
        {
            switch (controllerModel)
            {
                ...
                case Model.OculusTouch:
#if false
                    if (joystick_color_is_black)
                    {
                        /* set the joystick color to "black" again, which might end up adding
                         * or removing the joystick_scrollwheel_color */
                        SetComponentColor(EColorizableComponent.Joystick, Color.black);
                    }
#endif
                    break;
            }
...
#if false
        public enum EColorizableComponent
        {
            /* The numeric values are magic. See explanation in Baroque_OculusVRManager.LateUpdate */
            ControllerBackground = 0,
            TouchpadBorder = 1,
            TouchpadSurface = 3,
            MenuBorder = 4,
            MenuSurface = 12,
        ...
        static readonly Color joystick_scrollwheel_color = new Color(0.12f, 0.46f, 0.2f);
        bool joystick_color_is_black = true;
        ...
        public void SetComponentColor(EColorizableComponent component, Color color)
        {
            if (baroque_vr_manager is Baroque_OculusVRManager)
            {
                ...
                {
                    joystick_color_is_black = (color == Color.black);
                    if (joystick_color_is_black && scrollWheelVisible != 0)
                        color = joystick_scrollwheel_color;
                }
        ...
        string GetHintResourceBase()
        {
            switch (controller_model)
            {
                case Model.HTCVive:
                    return "BaroqueUI/ControllerHTCVive/Hint";
                case Model.OculusTouch:
                    return "BaroqueUI/ControllerOculusTouch/Hint" + (index == 0 ? "Left" : "Right");
            }
            return null;
        }
        ...
        void UpdateControllerHint(ref Hint hint, string resource_name, bool too_fast)
        {
            if (hint.text == null)
                return;
            ...
            else
            {
                if (hint.gobj == null || !hint.gobj)
                {
                    ...
                    var rbase = GetHintResourceBase();
                    if (rbase == null)
                    {
                        hintsShowAt = GetTime() + HINT_DELAY;
                        return;
                    }
                    GameObject prefab = Resources.Load<GameObject>(rbase + resource_name);
                    if (prefab != null)
                    {
                    ...
                    {
                        Debug.LogError("missing resource: " + rbase + resource_name);
                        return;
                    }
                }
        ...
        void UpdateCurrentPointer(GameObject prefab, string name)
        {
            ...
            if (pointer_object_prefab != null)
            {
                pointer_transform = Instantiate(pointer_object_prefab, hotspot).transform;
                pointer_transform.localPosition = Vector3.zero;
